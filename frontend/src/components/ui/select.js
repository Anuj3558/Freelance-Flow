...
export const SelectComponent = ({ label, error, options, className = "", ...props }) => {
    return (_jsxs("div", { className: "space-y-1", children: [label && _jsx("label", { className: "block text-sm font-medium text-gray-700", children: label }), _jsxs("div", { className: "relative", children: [_jsx("select", { className: `
            ...
            ${className}
          `, ...props, children: options.map((option) => (_jsx("option", { value: option.value, children: option.label }, option.value))) }), _jsx(ChevronDownIcon, { className: "absolute right-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-gray-400 pointer-events-none" })] }), error && _jsx("p", { className: "text-sm text-red-600", children: error })] }));
};
...
function SelectScrollUpButton({ className, ...props }) {
    return (_jsx(SelectPrimitive.ScrollUpButton, { "data-slot": "select-scroll-up-button", className: cn("flex cursor-default items-center justify-center py-1", className), ...props, children: _jsx(ChevronDownIcon, { className: "size-4" }) }));
}
